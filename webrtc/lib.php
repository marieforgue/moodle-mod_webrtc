...
/** example constant */
//define('webrtc_ULTIMATE_ANSWER', 42);
...
function webrtc_add_instance(stdClass $webrtc, mod_webrtc_mod_form $mform = null) {
    ...
    $webrtc->timemodified = $webrtc->timecreated;

    // process the wysiwyg editors
    //$webrtc->flux = $webrtc->flux['text'];
    //$webrtc->flux = FORMAT_HTML;
    $fromform = $mform->get_data();
    $webrtc->flux = $fromform->flux;

    // insert the new record so we get the id
    $webrtc->id = $DB->insert_record('webrtc', $webrtc);
